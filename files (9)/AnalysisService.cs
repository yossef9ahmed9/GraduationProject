...
namespace GraduationProject.Services.OCR
{
    public class AnalysisService : IAnalysisService
    {
        ...
        public AnalysisResult Analyze(string text)
        {
            ...
            // FIXED: removed EnsureCompleteTests entirely.
            // The old implementation padded every undetected test with Value = 0,
            // which caused every undetected test to trigger a false "Low" alert.
            // We now only report tests that were actually found in the image.
            // If the caller needs to know which tests are missing, they can compare
            // result.Tests against the full panel list themselves.

            if (!tests.Any())
            {
                result.Status = "Unknown";
                result.Alerts.Add("No lab values detected in the image. " +
                                  "Ensure the image is clear and contains a standard CBC panel.");
                return result;
            }
        ...
        private List<LabValue> ExtractTests(string text)
        {
            ...
            foreach (var line in lines)
            {
                ...
                // MCH special case
                var mchSpecialMatch = Regex.Match(clean,
                    @"Il\s+(\d+\.?\d*)\s+pg\s+(\d+\.?\d*)\s*-\s*(\d+\.?\d*)",
                    RegexOptions.IgnoreCase);
                ...
                var hbMatch = Regex.Match(clean,
                    @"haemoglobin\s*(\d+\.?\d*)\s*g/dl\s*(\d+\.?\d*)\s*-\s*(\d+\.?\d*)",
                    RegexOptions.IgnoreCase);
                if (hbMatch.Success)
                {
                ...
                var hctMatch = Regex.Match(clean,
                    @"haematocrit.*?(\d+\.?\d*)\s*%\s*(\d+\.?\d*)\s*-\s*(\d+\.?\d*)",
                    RegexOptions.IgnoreCase);
                if (hctMatch.Success)
                {
                    tests.Add(new LabValue
                ...
                var mchcMatch = Regex.Match(clean,
                    @"mchc\s*(\d+\.?\d*)\s*g/dl\s*(\d+\.?\d*)\s*-\s*(\d+\.?\d*)",
                    RegexOptions.IgnoreCase);
                ...
                var wbcMatch = Regex.Match(clean,
                    @"leucocytic.*?(\d+\.?\d*)\s*thousands.*?(\d+\.?\d*)\s*-\s*(\d+\.?\d*)",
                    RegexOptions.IgnoreCase);
                if (wbcMatch.Success)
                {
                    tests.Add(new LabValue
