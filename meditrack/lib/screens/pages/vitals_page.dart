...
class _VitalsPageState extends State<VitalsPage> {
  ...
  void initState() {
    ...
    _sensorRefresh = Timer.periodic(const Duration(seconds: 15), (_) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (_history.isNotEmpty) {
          _analyzeRisk(_history.first);           // fire and forget
        }
      } else {
  ...
  Future<void> _analyzeRisk(VitalSignsResponse vital) async {
    ...
    if (!mounted) return;
    if (res.ok && res.data != null) _handleRisk(res.data!);
  }

  void _handleRisk(HeartRiskResponse risk) {
    if (!mounted) return;
    setState(() => _riskResult = risk);

    if (risk.tier.toUpperCase() == 'WARNING') {
      // أصفر بسيط — لا EmergencyBanner
    ...
    // CRITICAL → EmergencyBanner يظهر أوتوماتيك من condition في build()
  }
  ...
  // Critical = emergencyStatus من الباك (قرار الـ AI) — مش قيم ثابتة
  // Warning  = قيم threshold لأن الباك مش بيحفظ warning flag
  bool _isWarning(VitalSignsResponse v) {
    if (v.emergencyStatus == true) return false; // Emergency مش Warning
    return v.heartRate > 110 ||
        v.heartRate < 55 ||
        (v.oxygenSaturation != null && v.oxygenSaturation! < 95.0);
  }

  String _vitalStatus(VitalSignsResponse v) {
    if (v.emergencyStatus == true) return 'Emergency';
    if (_isWarning(v))             return 'Warning';
    return 'Normal';
  }

  BadgeType _vitalBadgeType(VitalSignsResponse v) {
    if (v.emergencyStatus == true) return BadgeType.red;
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // CRITICAL فقط → EmergencyBanner أحمر — المصدر: emergencyStatus من الباك بعد قرار الـ AI
          // WARNING → SnackBar أصفر بس (من _handleRisk) مش EmergencyBanner
          if (latest != null && latest.emergencyStatus == true) ...[
            EmergencyBanner(text: _riskResult?.message ?? _alertText(latest)),
              ...
              if (_history.isEmpty)
                ...
                SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: _history.take(20).toList().asMap().entries.map((entry) {
                      final v   = entry.value;
