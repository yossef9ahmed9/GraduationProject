...
const _patientNav = [
  ...
  _NavItem('lab-request',  'Lab Tests',      Icons.science_outlined),
...
Widget _buildPage(String id, UserRole role) {
  switch (id) {
    ...
    case 'book-followup':return const DoctorsPage();
...
String _pageTitle(String id) {
  const titles = {'dashboard':'Dashboard','patients':'Patients','doctors':'Doctors','vitals':'Vital Signs',
    'labs':'Labs','tests':'Medical Tests','followups':'Follow-ups','ambulances':'Ambulances',
    'sensors':'Sensors','profile':'My Account','book-followup':'Doctors','lab-request':'Lab Tests'};
  return titles[id] ?? id;
}

class HomeNavigator {
  static _HomeScreenState? _of(BuildContext context) => context.findAncestorStateOfType<_HomeScreenState>();
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (width < 700) {
      return _MobileShell(nav: nav, role: role, user: auth.user, currentPage: _currentPage, onNavigate: navigateTo,
        child: _buildPage(_currentPage, role));
    }
    return _TabletShell(nav: nav, role: role, user: auth.user, currentPage: _currentPage, onNavigate: navigateTo,
      child: _buildPage(_currentPage, role));
  }
}
...
class _MobileShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(_pageTitle(currentPage)),
        actions: [
          Container(margin: const EdgeInsets.only(right: 8), width: 30, height: 30,
            decoration: BoxDecoration(color: isDark ? AppColors.darkAccentMuted : AppColors.accentMuted, shape: BoxShape.circle),
            child: InkWell(onTap: () => onNavigate('profile'),
              child: Center(child: Text(user?.initials ?? 'U', style: GoogleFonts.dmSans(fontSize: 11, fontWeight: FontWeight.w700,
                color: isDark ? AppColors.darkBadgeBlueTxt : AppColors.badgeBlueTxt))))),
        ]),
      drawer: _SideDrawer(nav: nav, role: role, user: user, currentPage: currentPage,
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: bottomItems.map((n) => NavigationDestination(icon: Icon(n.icon, size: 22), label: n.label)).toList()),
    );
  }
}
...
class _TabletShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(body: Row(children: [
      Container(width: 210,
        decoration: BoxDecoration(color: isDark ? AppColors.darkBgSidebar : AppColors.bgSidebar,
          border: Border(right: BorderSide(color: isDark ? AppColors.darkBorderColor : AppColors.borderColor))),
        child: Column(children: [
          Container(padding: const EdgeInsets.fromLTRB(16,16,16,14),
            decoration: BoxDecoration(border: Border(bottom: BorderSide(color: isDark ? AppColors.darkBorderColor : AppColors.borderColor))),
            child: Row(children: [
              Container(width: 28, height: 28, decoration: BoxDecoration(gradient: isDark ? AppColors.darkLogoGradient : AppColors.logoGradient, borderRadius: BorderRadius.circular(8)),
                child: const Icon(Icons.monitor_heart_outlined, color: Colors.white, size: 16)),
              const SizedBox(width: 8),
              Text('MediTrack', style: GoogleFonts.dmSans(fontSize: 14, fontWeight: FontWeight.w700, letterSpacing: -0.2)),
            ])),
      ...
      Expanded(child: Column(children: [
        Container(height: 52, padding: const EdgeInsets.symmetric(horizontal: 20),
          decoration: BoxDecoration(color: isDark ? AppColors.darkBgCard : AppColors.bgCard,
            border: Border(bottom: BorderSide(color: isDark ? AppColors.darkBorderColor : AppColors.borderColor))),
          child: Row(children: [
            Text(_pageTitle(currentPage), style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w600, letterSpacing: -0.2)),
            const Spacer(),
            InkWell(onTap: () => onNavigate('profile'), child: Row(children: [
              AvatarWidget(initials: user?.initials ?? 'U'),
              const SizedBox(width: 8),
              Text(user?.name ?? '', style: GoogleFonts.dmSans(fontSize: 13, fontWeight: FontWeight.w500)),
            ])),
          ])),
...
class _SideDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Drawer(backgroundColor: isDark ? AppColors.darkBgSidebar : AppColors.bgSidebar,
      child: Column(children: [
        DrawerHeader(child: Row(children: [
          Container(width: 36, height: 36, decoration: BoxDecoration(gradient: isDark ? AppColors.darkLogoGradient : AppColors.logoGradient, borderRadius: BorderRadius.circular(10)),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Text(user?.name ?? '', style: GoogleFonts.dmSans(fontSize: 12, color: isDark ? AppColors.darkTextSecondary : AppColors.textSecondary)),
          ]),
        ])),
        Expanded(child: _SideNav(nav: nav, currentPage: currentPage, onNavigate: onNavigate)),
...
class _SideNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    String? lastSection;
    return ListView(padding: const EdgeInsets.symmetric(vertical: 4), children: nav.map((item) {
      ...
      widgets.add(Material(color: Colors.transparent, child: InkWell(
        onTap: () => onNavigate(item.id),
        borderRadius: BorderRadius.circular(8),
        child: Container(margin: const EdgeInsets.symmetric(horizontal: 8, vertical: 1),
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 9),
          decoration: BoxDecoration(color: selected ? (isDark ? AppColors.darkAccentMuted : AppColors.accentMuted) : null, borderRadius: BorderRadius.circular(8)),
          child: Row(children: [
            Icon(item.icon, size: 16, color: selected ? (isDark ? AppColors.darkAccent : AppColors.accent) : (isDark ? AppColors.darkTextSecondary : AppColors.textSecondary)),
            const SizedBox(width: 9),
            Text(item.label, style: GoogleFonts.dmSans(fontSize: 13, fontWeight: selected ? FontWeight.w600 : FontWeight.w400,
...
class _LogoutButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Container(decoration: BoxDecoration(border: Border(top: BorderSide(color: isDark ? AppColors.darkBorderColor : AppColors.borderColor))),
      child: Material(color: Colors.transparent, child: InkWell(onTap: onTap,
        child: Padding(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), child: Row(children: [
          Icon(Icons.logout_rounded, size: 14, color: isDark ? AppColors.darkBadgeRedTxt : AppColors.badgeRedTxt),
          const SizedBox(width: 9),
